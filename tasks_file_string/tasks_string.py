...
def check_result_file(source_file_name):
    ...
    dir_name_source_file = os.path.dirname(os.path.abspath(f"tasks_file_string/{source_file_name}"))
...
def creator_dict_ip_day_of_week(list_log_line, popular_day_of_week):
    ...
    if not list_log_line[0] in popular_day_of_week:
        popular_day_of_week[list_log_line[0]] = [list_log_line[2]]
    popular_day_of_week[list_log_line[0]] = [*(popular_day_of_week[list_log_line[0]]), list_log_line[2]]
...
def analysis_log_site(source_file_name="task_4_statistics.txt"):
    ...
    result_file_path_write = check_result_file(source_file_name)
    all_time_from_log = []  # list of all timestamps of user activity
    ...
    for log_line in log_data:
        list_log_line = [unit.strip() for unit in log_line.split('|')]
        ip_usage_list.append(list_log_line[0].strip())
        all_time_from_log.append(((list_log_line[1].strip()).split(":"))[0])
        creator_dict_ip_day_of_week(list_log_line, popular_days_of_week)
    ...
    max_activity_on_day = hour_max_activity_in_day(all_time_from_log)
...
def creator_log_analysis_file(popular_day_of_week, ip_usage_dict, max_activity_on_day, result_file_path_write):
    ...
    for ip in popular_day_of_week:
        ...
        result_string = f"{ip} | most popular days of activity: {week_day} | " \
                        f"number of visits to the resource: {ip_count_in}\n\n"
        write_file(result_file_path_write, result_string)
    if int(max_activity_on_day[0][0]) < int(max_activity_on_day[1][0]):
        most_popular_hour = f"Most popular times of user activity from {max_activity_on_day[0][0]} " \
                            f"hour to {max_activity_on_day[1][0]} hour\n"
        write_file(result_file_path_write, most_popular_hour)
    most_popular_hour = f"Most popular times of user activity from {max_activity_on_day[1][0]} " \
                        f"hour to {max_activity_on_day[0][0]} hour\n"
    write_file(result_file_path_write, most_popular_hour)
